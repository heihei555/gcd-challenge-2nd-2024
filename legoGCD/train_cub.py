...
from data.augmentations import get_transform, get_transform2
...
from model import DINOHead, info_nce_logits, SupConLoss, DistillLoss, ContrastiveLearningViewGenerator, \
    get_params_groups, initial_qhat, update_qhat, causal_inference, WeightedEntropyLoss
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                # * Soft loss计算学生模型对未标记数据的预测结果，并根据最大概率来筛选高质量的预测
                ...
                mask_p_unlabeled_true_old = (mask_lab_p == False) & (mask_all == True) & (labels_p == idx) & (
                    idx.lt(args.num_labeled_classes))
                mask_p_unlabeled_true_novel = (mask_lab_p == False) & (mask_all == True) & (labels_p == idx) & (
                    idx.ge(args.num_labeled_classes))
                ...
                pstr += f'nll_loss: {nll_loss.mean().item():.4f} '  # 负对数似然损失，通常用于分类任务，尤其是在多类别分类中
            ...
            if batch_idx % args.print_freq == 0:
                args.logger.info(
                    'Epoch: [{}][{}/{}]\t loss {:.5f}\t {} Unlabeled condidate {} Unlabeled pred Old {} Unabeled true {} Unlabeled true_to_old {} Unlabeled true_to_novel {} Unabeled wrong {} Wrong old {} Wrong novel {} Wrong novel_to_old {} Wrong novel_to_novel {}'
                    .format(epoch, batch_idx, len(train_loader), loss.item(), pstr, Unlabeled_condiate, mask_old_num,
                            Unlabeled_true, Unlabeled_true_to_old, Unlabeled_true_to_novel, Unlabeled_wrong,
                            num_wrong_old, num_wrong_novel, num_wrong_novel_to_old, num_wrong_novel_to_novel))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', type=str, default='cub', help='options: cifar10, cifar100, imagenet_100, cub, scars, fgvc_aircraft, herbarium_19')
    parser.add_argument('--prop_train_labels', type=float, default=0.5)
    parser.add_argument('--use_ssb_splits', action='store_true', default=True)

    parser.add_argument('--grad_from_block', type=int, default=23)
    parser.add_argument('--lr', type=float, default=0.10001)
    ...
    parser.add_argument('--exp_name', default='cub_LegoGCD', type=str)
    ...
    init_experiment(args, runner_name=['legogcd_cub'])
    ...
    # TRAIN
    # ----------------------
    train(model, train_loader, test_loader_labelled, test_loader_unlabelled,device, args)
